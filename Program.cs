...
static async Task ModernTranslation()
{
  ...
  const string processedFilePath = "processed_bible_books.json";
  List<string> processedBooks = LoadProcessedBooks(processedFilePath);
  const string progressFilePath = "batch_progress.json";
  ...
  foreach (var file in githubFileList)
  {
    ...
    {
      bookVerses = await GitHubService.LoadKjvBibleBookJsonAsync(file.download_url, file.name);
    }, maxRetries);
    ...
    for (int i = startIndex; i < bookVerses.Count; i += batchSize)
    {
      ...
      bool batchSuccess = await RetryAsync(() => AiService.TranslateBibleVersesToModernEnglishAsync(batch), maxRetries);
      if (!batchSuccess)
      {
